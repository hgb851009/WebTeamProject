...
<div id="s-container">
<c:if test="${loginuser.classNo eq bkind || loginuser.level eq '관리자'}">
<c:choose>
...
<h1 align="center">${bkind }반 게시판</h1>
</div>
...
<td align="left">
<a href="b_readCountUp.do?bno=${vo.bno}&page=${pInfo.curPage}&criteria=${search.criteria}&keyword=${search.keyword}">${vo.title} <sub>(${vo.replyCount })</sub></td>
...
<c:if test="${pInfo.curPage>1 }">
	<c:choose>
		<c:when test="${search.criteria==null }">
			<a href="board.do?page=${pInfo.curPage-1 }"><</a>
		</c:when>
	<c:otherwise>
<a href="boardSearch.do?page=${pInfo.curPage-1 }&criteria=${search.criteria }&keyword=${search.keyword }"><</a>
	</c:otherwise>
	</c:choose>
</c:if>
...
<script>
$('input:radio[name=boardKind]:input[value=${bkind }]').attr('checked',true);
...
function view(k){
 	...
 	formObj.attr("action", "boardKindList2.do");
 	formObj.submit();	
 };
 function view0(k){
	 	$("input[name='boardkind']").val(k);
